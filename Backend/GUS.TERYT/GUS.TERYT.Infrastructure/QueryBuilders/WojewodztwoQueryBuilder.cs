...
public class WojewodztwoQueryBuilder(TerytDbContext context) : BaseQueryBuilder<Wojewodztwo>(context.Wojewodztwa)
{
    ...
    public WojewodztwoQueryBuilder WithPagination(Pagination pagination, Order order, WojewodztwoOrderBy orderBy)
    {
        With(query =>
        {
            ...
            {
                WojewodztwoOrderBy.Name => order == Order.Descending
                    ? query.OrderBy(i => i.Name)
                    : query.OrderByDescending(i => i.Name),

                WojewodztwoOrderBy.Id => order == Order.Descending
