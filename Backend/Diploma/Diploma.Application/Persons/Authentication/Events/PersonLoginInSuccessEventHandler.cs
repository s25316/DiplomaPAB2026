...
namespace Diploma.Application.Persons.Authentication.Events;

public class PersonLoginInSuccessEventHandler(
    IEmailService emailService,
    IEventPublisher<PersonLoginInSuccessEvent> publisher,
    IPersonOperationRepository operationRepository,
    IPersonLoginInSuccessMessageGenerator messageGenerator
    ) : INotificationHandler<PersonLoginInSuccessEvent>
{
    public async Task Handle(PersonLoginInSuccessEvent notification, CancellationToken cancellationToken)
    {
    ...
    private async Task<PersonOperationId> CreateOperationAsync(
        PersonLoginInSuccessEvent notification,
    ...
    ) => await operationRepository.CreateAsync(new PersonOperationInput.Creating
    {
        ...
        PersonOperation = PersonOperation.LogInSucess,
    }, cancellationToken);
