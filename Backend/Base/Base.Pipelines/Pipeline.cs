...
using Base.Pipelines.Interfaces.Pipelines;
    ...
    ISyncPipeline<TInput, TOutput>
{
    ...
    public virtual Pipeline<TInput, TOut> Add<TOut>(
    ...
    public virtual async Task<PipelineResult<TOutput>> ExecuteAsync(TInput input, CancellationToken cancellationToken = default)
    {
        var result = await function(input, cancellationToken).ConfigureAwait(false);
        if (result.HasException) throw result.Exception;
        return result;
    }
    public PipelineResult<TOutput> Execute(TInput input) => ExecuteAsync(input, CancellationToken.None).GetAwaiter().GetResult();
}
